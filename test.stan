...
parameters { 
  ...
  vector[M] beta; // eta
  real phi;
  real log_initial_inf; // initial infectiousness
...
transformed parameters {
  ...
  infectiousness = rep_vector(1e-5, t);
  
  infectiousness[1] = exp(log_initial_inf);
  infections[1] = infectiousness[1] * R[1];
  
  for (s in 2:t) {
    infectiousness[s] += dot_product(infections[max(1, (s - max_gt)):(s -1)],
    tail(rev_generation_time, min(max_gt, s - 1)));
    infections[s] = R[s] * infectiousness[s];
  }
...
model {
  ...
  beta ~ std_normal();
  phi ~ exponential(1);
  cases ~ neg_binomial_2(reports, inv(phi));
  log_initial_inf ~ std_normal();
